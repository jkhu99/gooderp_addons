...
class MoneyInvoice(models.Model):
    ...
    ref = fields.Char(string=u'客户订单号', help=u'客户订单号')					   
    ...
    def money_invoice_done(self):
        ...
        for inv in self:
            ...
            # inv.date_due = datetime.datetime.strptime(inv.date, '%Y-%m-%d') + datetime.timedelta(days=inv.partner_id.credit_time)
            invdate_datatime=datetime.datetime.strptime(inv.date, '%Y-%m-%d')
            inv.date_due = invdate_datatime - datetime.timedelta(days=invdate_datatime.day) + datetime.timedelta(days=30) + datetime.timedelta(days=inv.partner_id.credit_time)
